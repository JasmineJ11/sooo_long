...
/*   Updated: 2025/08/07 16:48:07 by jiawli           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void element_counts(t_parameter_check *checker)
{
	if(checker->entrance  != 1)
		exit_prog(NULL, "Map need 1 player.");
	if(checker->collectible <= 0)
		exit_prog(NULL, "Map need at least 1 collectible.");
	if(checker->exit != 1)
		exit_prog(NULL, "Map need 1 exit.");
...
static void element_process(char **parameter, t_parameter_check *checker, int i, int j)
{
	if (parameter[i][j] != '1' && parameter[i][j] != '0'
		...
		&& parameter[i][j] != 'P')
		exit_prog(&parameter, "Illegal element in map.");
	if ((i == 0 || i == checker->height - 1 || j == 0 || j == checker->length - 1)
		&& parameter[i][j] != '1')
		exit_prog(&parameter, "Map should be surrounded by walllls");
	if (parameter[i][j] == 'C')
		checker->collectible++;
	if (parameter[i][j] == 'E')
		checker->exit++;
...
static void dimensions(char **parameter, t_parameter_check *checker)
{
	...
	if(checker->height > 130 || checker->length > 130)
		exit_prog(&parameter,"Map dimensions too large (max 130x130).");
	if(checker->height < 3 || checker->length < 3)
		exit_prog(&parameter, "Map too small.");
	...
	while(i < checker->height)
	{
		...
		if(len == 0)
			exit_prog(&parameter, "Map has empty line.");
		if(len != checker->length)
			exit_prog(&parameter, "Map is wrong wrong shape.");
		i++;
	}
}
