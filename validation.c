...
static void	element_process(char **parameter, t_parameter_check *checker, int i,
		int j)
{
	if (parameter[i][j] != '1' && parameter[i][j] != '0'
		...
		exit_prog(&parameter, NULL, NULL,
			"Illegal element in map. What did you put");
	if ((i == 0 || i == checker->height - 1 || j == 0 || j == checker->length
			- 1) && parameter[i][j] != '1')
		exit_prog(&parameter, NULL, NULL,
			"Map should be surrounded by walllls");
	if (parameter[i][j] == 'C')
		checker->collectible++;
	if (parameter[i][j] == 'E')
		checker->exit++;
